...
public class OutboxPublisher {
    ...
    @Scheduled(initialDelayString = "${marketplace.outbox.initial-delay:1s}", fixedDelayString = "${marketplace.outbox.fixed-delay:1s}")
    @Transactional
    public void publishPending() {
        for (OutboxRecord event : claimBatch()) {
            try {
                publish(event);
                jdbcTemplate.update(
                    "UPDATE event_outbox SET published_at = now(), locked_at = NULL, last_error = NULL WHERE event_id = ?",
                    event.eventId()
                );
            } catch (RuntimeException exception) {
                jdbcTemplate.update(
                    "UPDATE event_outbox SET attempts = attempts + 1, locked_at = NULL, last_error = ? WHERE event_id = ?",
    ...
    private void publish(OutboxRecord event) {
        ...
        rabbitTemplate.convertAndSend(event.exchange(), event.routingKey(), event.payload(), correlation);
        try {
            CorrelationData.Confirm confirm = correlation.getFuture().get(5, TimeUnit.SECONDS);
            if (!confirm.ack()) throw new IllegalStateException("Broker rejected event: " + confirm.reason());
            if (correlation.getReturned() != null) throw new IllegalStateException("Event had no matching queue");
        ...
        return jdbcTemplate.query(
            """
                WITH claimed AS (
                    ...
                    UPDATE event_outbox AS outbox SET locked_at = now()
                ...
                FROM updated ORDER BY occurred_at, event_id
                """,
    ...
    private static String abbreviate(String message) {
        if (message == null) return "Unknown broker error";
        return message.substring(0, Math.min(message.length(), 500));
    }

    private record OutboxRecord(UUID eventId, String exchange, String routingKey, String payload) {
    }
}
